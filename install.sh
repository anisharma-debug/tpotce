...
mySUPPORTED_DISTRIBUTIONS=("AlmaLinux" "Debian GNU/Linux" "Fedora Linux" "openSUSE Tumbleweed" "Raspbian GNU/Linux" "Rocky Linux" "Ubuntu" "Oracle Linux" "Oracle Linux Server")
myCURRENT_DISTRIBUTION=$(awk -F= '/^NAME/{print $2}' /etc/os-release | tr -d '"')

if [[ ! " ${mySUPPORTED_DISTRIBUTIONS[@]} " =~ " ${myCURRENT_DISTRIBUTION} " ]];
  then
  echo "### Only the following distributions are supported: AlmaLinux, Fedora, Debian, openSUSE Tumbleweed, Rocky Linux, Ubuntu, Oracle Linux, and Oracle Linux Server."
...
case ${myCURRENT_DISTRIBUTION} in
  ...
  "openSUSE Tumbleweed")
    ...
    ;;
  "AlmaLinux"|"Rocky Linux"|"Oracle Linux"|"Oracle Linux Server")
    echo
    echo ${myINSTALL_NOTIFICATION}
    echo
...
myANSIBLE_DISTRIBUTIONS=("Fedora Linux" "Debian GNU/Linux" "Raspbian GNU/Linux" "Rocky Linux" "Oracle Linux" "Oracle Linux Server")
